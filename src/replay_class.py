...
import struct
from pymavlink.dialects.v20 import common as mavlink2
# assumes you already have rebuild_messages_from_bytes(data) defined
...
def rebuild_messages_from_bytes(data: bytes) -> Iterator[Tuple[Optional[object], float]]:
    """
...
# # --- In-memory record iterator (bytes -> (frame_or_none, ts)) ---
# def iter_recvmatch_records_from_bytes(data: bytes) -> Iterator[Tuple[Optional[bytes], float]]:
#     mv, off, n_total = memoryview(data), 0, len(data)
#     while off < n_total:
#         if off + HDR_SZ > n_total:
#             raise EOFError("truncated record header")
#         rtype, ts_usecs, n = struct.unpack_from(HEADER_FMT, mv, off)
#         if rtype not in (REC_NONE, REC_FRAME):
#             raise ValueError(f"unknown record type {rtype} at {off}")
#         off += HDR_SZ

#         ts = ts_usecs / 1_000_000.0  # float seconds (wall clock)
...
#         msg = None
#         # Feed entire frame; keep last non-None to ensure full consume (incl. v2 signatures)
#         for b in frame_or_none:
#             out = parser.parse_char(bytes([b]))
#             if out is not None:
#                 msg = out
...
class ReplayMode:
    ...
    def __init__(self, data: bytes):
        ...
        self._entries: List[ReplayEntry] = [
            ReplayEntry(msg, ts) for (msg, ts) in rebuild_messages_from_bytes(data)
        ]
