...
RECV_LOG_PATH = os.environ.get("RECV_LOG_PATH", "recv.bin")
SEND_LOG_PATH = os.environ.get("SEND_LOG_PATH", "send.bin")
...
def truncate_logs():
    try:
        open(RECV_LOG_PATH, 'w').close()
        open(SEND_LOG_PATH, 'w').close()
        # print(f"[Supervisor] [Logs]: Logs truncated.")
    except Exception as e:
        print(f"[Supervisor] [Logs]: Failed to truncate: {e}")
...
def run_checkpoint_loop():
    ...
    while True:
        try:
            if app2_pid: os.kill(app2_pid, 0)
            else: 
        ...
        with master_lock:
            ...
            try:
                os.kill(app2_pid, signal.SIGSTOP)
                subprocess.run(["criu", "dump", "-t", str(app2_pid), "-D", CHECKPOINT_DIR, "--shell-job", "--leave-running", "--tcp-close", "--ext-unix-sk", "--skip-in-flight", "-v0", "-o", "dump_criu.log"], check=True)
                print("[Supervisor] [Checkpointer]: Snapshot SUCCESS.")
                snapshot_success = True
            except Exception as e:
                print(f"[Supervisor] [Checkpointer]: FAILED: {e}")
            finally:
                os.kill(app2_pid, signal.SIGCONT)

            # if snapshot_success: truncate_logs()
